...
module.exports = function (app, db) {
  ...
  function findFromUrlsCol (query, cb) {
    urlsCol.findOne(query, { fields: { original_url: 1, short_url: 1, _id: 0 } }, cb)
  }

  function incrementCounter (cb) {
    countersCol.findOneAndUpdate({ _id: 'url_count' }, { $inc: { seq: 1 } }, { returnOriginal: false }, cb)
  }
  ...
  app.get(apiPath + '*', (req, res, next) => {
    ...
    if (urlObj.protocol && urlObj.host) {
      findFromUrlsCol({ 'original_url': urlObj.href }, (err, doc) => {
        if (err) return next(err)
        if (doc) {
          res.json(doc)
        } else {
          incrementCounter((err, counter) => {
            if (err) return next(err)
            encode(counter.value.seq, (encodedSeq) => {
  ...
  app.get('/:encodedPath', (req, res, next) => {
    findFromUrlsCol({ 'short_path': req.params.encodedPath }, (err, doc) => {
      if (err) return next(err)
      if (doc) {
        res.json(doc)
      } else {
